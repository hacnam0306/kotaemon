...
class IndexDocumentPipeline(BaseFileIndexIndexing):
    ...
    def readers(self):
        ...
        elif self.reader_mode == "custom":
            readers[".pdf"] = custom_reader
    ...
    def get_user_settings(cls):
        return {
            ...
            "chunking_mode": {
                ...
                "value": "default",
                "choices": [
                    ("TokenSplitter Kotaemon", "default"),
                    ("Fixed-Size Text Chunking", "fix_size"),
                    ("Semantic Chunking", "semantic"),
                    ("Recursive Document Chunking", "recursive"),
                    ("LLM-Based Chunking", "llm_chunking"),
                    ("Document Structure-Based Chunking", "doc_structure"),
                ],
                "component": "dropdown",
            },
        }

    def get_chunker_from_mode(self, chunk_size=None, chunk_overlap=None):
        """Return an appropriate chunker based on the chunking mode
        ...
        Returns:
            BaseSplitter: A splitter instance
        """
        # Use defaults if not provided
        chunk_size = chunk_size or 1024
        chunk_overlap = chunk_overlap or 256
        separators = ["\n\n", "\n", ".", "\u200B"]  # Default separators
        print(f"{self.chunking_mode=}, {chunk_size=}, {chunk_overlap=}")
        # Choose the appropriate chunker based on mode
        if self.chunking_mode == "default":
            # Default TokenSplitter from kotaemon
    ...
    def get_pipeline(cls, user_settings, index_settings) -> BaseFileIndexIndexing:
        ...
        obj = cls(
            ...
            chunking_mode=user_settings.get("chunking_mode", "default"),
        )
        return obj
    ...
    def route(self, file_path: str | Path) -> IndexPipeline:
        ...
        pipeline: IndexPipeline = IndexPipeline(
            loader=reader,
            splitter=self.get_chunker_from_mode(chunk_size, chunk_overlap),
